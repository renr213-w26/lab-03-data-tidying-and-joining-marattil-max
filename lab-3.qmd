...
**There are 44 rows, each row represents a specific country's inflation. There are 30 columns, each column, aside from the first, represents the inflation rate of the given year for all countries specified in the row**

```{r}
#b
country_inflation |> pull(1)
```
...
```{r}
top_three <- country_inflation |>
  select(country, `2021`)  |>
  slice_max(order_by = `2021`, n = 3) #slice_max is ordered by desending 

top_three
```

**All the top three inflation rates are more than double the inflation rate of the United States in 2021, with the top two inflation rates from Argentina and Türkiye being significantly greater than all the other countries in that year.**
...
**New Zealand has the highest ratio, thus it has the greatest change in the given time frame . This is seen by how the inflation rate for New Zealand has increase from 1993 to 2021.**
...
```{r}
#pivot_wider is less row, more col
#pivot_longer is more row, less col

country_yr <-country_inflation |>
  pivot_longer(
    cols = -country,                  # not needed to use coluntry bc it is index
    names_to = "year",                # all columns become row
    values_to = "annual_inflation",   # values in the year columns group into new column
...
**The resulting dataframes has 3 columns and 1276 rows.**
...
a.  What is the highest inflation rate observed between 1993 and 2021? The output of the pipeline should be a data frame with one row and three columns. In addition to code and output, your response should include a single sentence stating the country and year.
    ...
    #a
    country_yr |>
      filter (annual_inflation == max(annual_inflation, na.rm = TRUE)) #show only max value
    ```
...
b.  What is the lowest inflation rate observed between 1993 and 2021? The output of the pipeline should be a data frame with one row and three columns. In addition to code and output, your response should include a single sentence stating the country and year.
    ...
    #b
    country_yr |>
      filter (annual_inflation == min(annual_inflation, na.rm = TRUE)) #show only min value
    ```
...
```{r}
countries_of_interest <- c("Türkiye", "United States") #c creates vector
```
...
```{r}
countries_of_interest <- c("Brazil", "Ireland", "Argentina", "New Zealand")
```

**I chose:**

-   **Brazil: to examine its change in inflation rate, since its high point in 1994**
-   **Ireland: to examine its change in inflation rate, since its low point in 2009**
-   **Argentina : to examine the increase of inflation rate to allow it the greatest inflation rate of 2021**
-   **New Zealand: to examines its high change of inflation rate over the years**
...
```{r}
interested_countries <- country_yr|>
...
```{r}
ggplot(interested_countries, aes(x = year, y = annual_inflation, colour = country, shape = country)) + 
  geom_line()+
  geom_point() +
  #facet_wrap(~country) +
  labs(
    title = "Annual Inflation Rates per Year, seperated by Country",
    x = "Year",
    y = "Annual Inflation"
  ) +
  scale_y_continuous(limits = c(-5, 50)) +
  scale_x_continuous(breaks = seq(1993, 2021, by = 5))
```

**As expected, Argentina has a significantly higher inflation rate for the latest 3 year recorded, and it seem to have missing data for the previous year. Ireland had a dip into negative inflation rate for a year. I had expected to see a general increased rate of inflation however, while individual years have extreme rates of inflation, New Zealand had the most the obvious increase.**
...
#column information
ncol(us_inflation) 
colnames(us_inflation)

#row information
nrow(us_inflation) 
row.names(us_inflation)

```

**There are 4 columns containing country of origin, CPI division, year, and inflation rate of that given year, respectively. There are 132 rows, which are entry for the annual inflation rate per CPI division in the US.**
...
#column information
ncol(cpi_divisions) 
colnames(cpi_divisions)

#row information
nrow(cpi_divisions) 
row.names(cpi_divisions)
```

**There are 2 columns and 12 rows. The index, which is also one of the columns, is an assigned value entry for the 12 CPI division in the second column containing the descriptions of the divisions.**
...
```{r}
us_inflation_cpi <-left_join(
  us_inflation, cpi_divisions,
  by = c("cpi_division_id"= "id")
  )

```

**There were 5 columns and 132 rows. Due to join, the new table shows the same table as us_inflation with an added row that matches the cpi_division_id column from us_inflation with id column from cpi_division to create a column that has a description of the cpi division for the given entry specified in cpi_division_id.**
...
```{r}
divisions_of_interest <- c("Education", "Furnishings, household equipment and routine household maintenance","Communication")
```
...
```{r}
interested_division <- us_inflation_cpi|>
...
```{r}
ggplot(interested_division, aes(x = year, y = annual_inflation, colour = description, shape = description)) +
  geom_line()+
  geom_point() +
  theme_linedraw()+
  labs(
    title = "Annual Inflation Rates per Year, seperated by CPI Divisions",
    x = "Year",
    y = "Annual Inflation",
    color = "CPI Division",
    shape = "CPI Division"
...
**For, (Furnishings, household equipment and routine household maintenance) and (Communication) there is general trend of increase through the years. While Education had a decreasing trend for annual inflation. This decrease,show the decrease in money towards the Education division. Additionally, it was surprising to note the serevre dip in the trend for Communication.**
